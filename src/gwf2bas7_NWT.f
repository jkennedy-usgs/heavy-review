...
C3------PRINT A MESSAGE IDENTIFYING THE BASIC PACKAGE.
      WRITE(IOUT,1)INBAS
    1 FORMAT(1X,/1X,'BAS -- BASIC PACKAGE, VERSION 1.1.5, 9/10/2019',
...
C1------Check for existence of discretization file
      INDIS=IUNIT(IUDIS)
      IF(INDIS.LE.0) THEN
         WRITE(IOUT,*) ' DIS file must be specified for HEAVY to run'
         CALL USTOP(' ')
      END IF
      WRITE(IOUT,11) INDIS
...
60        FORMAT(34X,' ',A,/,
     &             6X,'U.S. GEOLOGICAL SURVEY MODELING TOOL FOR',
     &             ' FORWARD MODELING MICROGRAVITY WITH MODFLOW',/,
     &             A,'VERSION ',A,/,20X,'FOR MODFLOW-2005 AND '
     %             ,'MODFLOW-NWT'/)
...
C  Get next file name
      ...
      END IF
C
C7------SKIP FILES NOT NEEDED FOR HEAVY
      IF (ANY((/ 'CHD ', 'RCH ','ETS ', 'WEL ', 'RIV ',
     &             'DRN ', 'PCG ', 'MNW2', 'HOB ' /) == FILTYP)) THEN
        WRITE(IOUT, 61) FILTYP
61      FORMAT(1X, 'FILE SKIPPED, NOT NEEDED FOR HEAVY: ',A)
        GO TO 10
      END IF
        ...
        DO 20 I=1,NIUNIT
           IF(LINE(ITYP1:ITYP2).EQ.CUNIT(I)) THEN
              ...
              ELSEIF ( LINE(ITYP1:ITYP1+4).EQ."IRED" ) THEN
                  ...
                  ACCARG=ACCESS
                  GO TO 30
              ELSE IF (LINE(ITYP1:ITYP1+4).EQ."HEAD" ) THEN
                  IUNIT(I)=IU
                  FMTARG=FORM
                  ACCARG=ACCESS
                  GO TO 30
