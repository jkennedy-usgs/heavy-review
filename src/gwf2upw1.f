...
! Cell property data
        ...
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:) ::SC1UPW
...
!1------IDENTIFY PACKAGE AND INITIALIZE.
 ...
 9001 FORMAT (1X, /' UPW1 -- UPSTREAM WEIGHTING FLOW PACKAGE, ',
     +       'VERSION 1.1.5, 9/10/2019', /, 9X, 'INPUT READ FROM UNIT',
     +        I3,/)
...
!
C
C------CHECK IF ANY OBS PACKAGES ARE ACTIVE AND IPHDRY>0
      OBACTIVE = .FALSE.
      IF ( IPHDRY > 0 ) THEN
...
C5------ALLOCATE MEMORY FOR ARRAYS.
      ...
      END IF
      IF(ITRSS.NE.0) THEN
         ALLOCATE(SC1UPW(NCOL,NROW,NLAY))
      ELSE
         ALLOCATE(SC1UPW(1,1,1))
      END IF
      SC1UPW = 0.0
...
!
!  2021-11-01
!  Even though these aren't necessary for Heavy, it's easier to leave them
!  in, because they're responsible for advancing the current line through
!  the UPW file until you get to the storage parameters.
!
...
C7D-----DEFINE SPECIFIC STORAGE OR STORAGE COEFFICIENT IN ARRAY SC1 IF TRANSIENT.
      IF(ITRSS.NE.0) THEN
         IF(NPSS.EQ.0) THEN
            CALL U2DREL(SC1UPW(:,:,KK),STOTXT,NROW,NCOL,KK,IN,IOUT)
         ELSE
            READ(IN,*) LAYFLG(3,K)
            WRITE(IOUT,121) STOTXT,K,LAYFLG(3,K)
            CALL UPARARRSUB1(SC1UPW(:,:,KK),NCOL,NROW,KK,'SS',
     1           IOUT,STOTXT,LAYFLG(3,KK))
            IF(NOPCHK.EQ.0) CALL UPARARRCK(BUFF,IBOUND,IOUT,K,NCOL,
     1           NLAY,NROW,'SS  ')
         END IF
         IF(ISFAC.EQ.0) THEN
            CALL SGWF2UPWSC(SC1UPW(:,:,KK),KK,1)
         ELSE
            CALL SGWF2UPWSC(SC1UPW(:,:,KK),KK,0)
         END IF
      END IF
C
...
C7E-----IS CONVERTIBLE.
     ...
     1           NCOL,NLAY,NROW,'SY  ')
            END IF
            ! Computes Storage Capacity (SY * dr * dc)
...
C     ------------------------------------------------------------------
     ...
     1                      BUFF,BOTM,LBOTM,IOUT
      ...
      USE GWFUPWMODULE,ONLY:IUPWCB,SC1UPW,SC2UPW,So,Sn,IBOUND2,LAYTYPUPW
...
! Deallocate UPW data.
        ...
        DEALLOCATE(Gwfupwdat(IGRID)%SC1UPW)
