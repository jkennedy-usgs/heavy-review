...
!
!
SUBROUTINE UPARARRRP(IN,IOUT,NP,ILFLG,PTYP,ITERP,ITVP,IACT,PN)
! ******************************************************************
...
! ******************************************************************
! Read a list parameter name, look it up in the list of
! parameters, and return its location.
! ******************************************************************
!
! SPECIFICATIONS:
! ------------------------------------------------------------------
...
!5------Set the active flag.
          IACTIVE(IP)=NI
!
!6------Compute pointers to the beginning and ending location of the
!6------parameter list within the package list of data.
...
!7------Return.
  ...
  100 CONTINUE
!
!8------All parameter names have been checked without finding the
!8------parameter. Write an error message and stop.
      WRITE(IOUT,*) ' The ',PACK, &
     &   ' file specifies an undefined parameter:',LINE(ISTART:ISTOP)
      CALL USTOP(' ')
!
...
! ******************************************************************
! Find the parameter number for a parameter name
! ******************************************************************
!
! SPECIFICATIONS:
! ------------------------------------------------------------------
...
!1------Abort if parameter name is blank.
      ...
      END IF
!
!2------Look for the parameter name in the list of parameters.
      CTMP1=PNAME
      CALL UPCASE(CTMP1)
      DO IP=1,MXPAR
        CTMP2=PARNAM(IP)
      ...
      END DO
!
!3------Failed to find the name -- abort.
      WRITE(IOUT,20) CPACK
   10 FORMAT(1X,'Parameter type conflict:',/,1X,'Named parameter:',A, &
     &       ' was defined as type:',A,/,1X, &
     &       'However, this parameter is used in the ',A, &
     &       ' file, so it should be type:',A)
